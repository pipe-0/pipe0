...
// // Grab the single operation
// const operation = oas.operation("/v1/pipes/run", "post");

// // Supply extra request data if you want to override examples
// const { code } = oasToSnippet(
//   oas,
//   operation,
//   {
//     body: { name: "Bob", email: "bob@example.com", x: 5 }, // example request
//   },
//   {
//     bearerAuth: "<TOKEN>",
//   },
    ...
    pipes: [
      {
        ...
        config: {
          prompt: {
            text: `
    Prompt templates are dynamic. You can use fields that exist in your request, like {{ input name required="true" type="string" }}.
    
    Try to give the LLM clear instructions, for example: Find out if {{ input name required="true" type="string" }} is a good ICP fit for our company.

    Our company, <YOUR_COMPANY_NAME>, produces <ADD_COMPANY_DESCRIPTION>.

    Use the current job title: {{ input job_title required="true" type="string" }} to make your assessment.
