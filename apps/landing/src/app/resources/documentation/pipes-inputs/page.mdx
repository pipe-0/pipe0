...
If we find **at least one input object** that can be processed, <AppLink linkType="pipelineValidation">pipeline validation</AppLink> will pass.
...
No input object has the required field `company_websiste_url`. The request will fail during <AppLink linkType="pipelineValidation">pipeline validation</AppLink>.
...
```
Some interactions require you reprocess previously processed fields. For this it is common to transform your output records
back to `input objects`. However, by doing so a lot of the previous processing information is lost. This includes metadata like 
...
Instead of passing your input as a plain value there is another way. You may pass your inputs fully or partially expanded (as the field value of the response object).
...
Expanding inputs gives you a great deal of control but shifts the responsibility of providing valid input states to you.
