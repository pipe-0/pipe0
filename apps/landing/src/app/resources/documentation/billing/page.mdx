...
If tasks fail, processed records are billed until the point of failure.

Enrichment requests in <AppLink linkType="testMode">sandbox</AppLink> mode are free.
...
Searches are billed per successful result. This includes results that are removed due to deduplication.

To receive the full result set (without deduplication), you may disable deduplication.

If a search fails, it is not billed. If a search partially completes, all found records are billed.
