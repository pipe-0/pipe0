...
At times, you may run into situations where some of your input fields conflict with pipe output fields.
Every field must have a clear owner. In cases like these, you have three options:
1. Rename our input field
2. Rename the pipe output field
3. Disable the pipe output field

##### Rename pipe output

All pipes who's output fields are known (`field mode = static`) before you add them allow you to rename or disable 
output fields. To rename output fields use the `config.output_fields[<FIELD_NAME>].alias` configuration.
...
##### Disable/toggle pipe output

To disable/enable a pipe field, use the `config.output_fields[<FIELD_NAME>].enabled` option.
