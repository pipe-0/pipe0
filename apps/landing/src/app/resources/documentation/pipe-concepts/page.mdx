...
(1) Pipes and their order (2) Fields added by your input
(3) Output fields added by pipes (4) Logic, conditions, and actions.
...
1. Your pipeline cannot contain self-referencing pipes (the input of a pipe points to its output)

### Pipeline requests

Internally, we convert the <AppLink linkType="pipelineRequest">request object</AppLink> into a pipeline.

### Pipeline response

After pipeline validation and processing, we convert a pipelines into the <AppLink linkType="pipelineResponse">response objects</AppLink>.