...
This pipe sends an email [Resend](https://resend.com). To use this pipe, reference a Resend API key in your request.

## Custom connections

Custom connections are credentials that you create and manage with external providers. To use custom connections, share 
your credentials (ex., an API key) with pipe0.
...
curl -X POST "https://api.pipe0.com/v1/pipes/run" \
  ...
  -d '{
    "pipes": [
      {
        "pipe_id": "action:sendmail:resend@1",
        "connector": {
          "strategy": "first",
          "connections": [
            {
