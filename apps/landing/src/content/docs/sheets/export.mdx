...
Writing to an outside system is a pipe column, the same as any enrichment. `object:update:hubspot@1`, `record:update:salesforce@1`, and `record:update:attio@1` each take a row's fields and update the matching CRM record. The cell records the outcome per row: `completed` rows are in the CRM, `failed` rows are not, and you can see which is which at a glance.
...
<Callout type="warn">
Test external writes in the sandbox environment first. Sandbox runs mock providers and never touches your CRM.
</Callout>
