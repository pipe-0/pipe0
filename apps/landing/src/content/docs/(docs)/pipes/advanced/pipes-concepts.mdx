...
**Example:** `people:workemail:waterfall@1` reads `name` + `company_website_url` and writes `work_email`.
...
> Run `people:name:split@1` if `name` is `completed` and its value contains `Tom`.
