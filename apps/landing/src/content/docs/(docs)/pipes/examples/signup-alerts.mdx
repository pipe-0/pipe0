...
Reason why it is a high value customer: {{ reason | default: "" }}
    ...
    },
    {
      ...
      connector: {
        strategy: "first",
        connections: [{ type: "vault", connection: "<ADD_YOUR_CONNECTION>" }],
      },
      // `message:write@1` stored its output in the `message` field.
      config: { channel_id: "<ADD_SLACK_CHANNEL_ID>", message: "{{ message }}" },
    },
  ],
...
Replace `<ADD_YOUR_CONNECTION>` with your Slack vault connection ID and `<ADD_SLACK_CHANNEL_ID>`
with the target channel (see [Connections](/docs/connections)).
