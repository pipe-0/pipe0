...
Not every template supports outputs. [prompt:run@1](/docs/pipe-catalog/prompt:run/1) requires at least one output tag; input tags are optional, so a static prompt with one `{% output %}` is valid. [email:write@1](/docs/pipe-catalog/email:write/1) supports only input tags.
...
| `type` | yes | `"string"`, `"number"`, `"boolean"`, or `"json"`. Not validated at save time, so typos fail late; stick to these four. |
| `description` | yes | Shown to the model. |
| `required` | no | `true` / `false`. Defaults to `true`. |
| `format` | no | A field format such as `"email"`, `"url"`, `"date"`, `"int"`, or `"markdown"`. |
