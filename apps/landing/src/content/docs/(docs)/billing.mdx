...
To enroll in high-volume usage for a specific pipe to to `dashboard` -> `billing` -> `High usage billing`.

### Custom discount levels

We're open to adding custom discounts for customers comitting to higher volumes. Reach out to [support@pipe0.com](mailto:support@pipe0.com).
