...
With high-usage billing you can opt into a discounted per-unit pricing.

You can check your plan features to see if your plan offers high usage quota.
...
To enroll in high-volume usage for a specific pipe go to `dashboard` -> `billing` -> `High usage billing`.
