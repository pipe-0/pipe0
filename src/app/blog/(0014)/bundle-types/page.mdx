...
can be confusing. Yet, it can help you not only when creating your own libraries, but also when 
you are consuming 3rd-party code.
...
If you explore types of an unbundled library e.g. by `cmd + click` in vscode - your IDE will take you
...
If you explore the types of of a bundled library you navigate through one large file. Let's explore when it makes
sense to bundle your TypeScript types.

## Tl;dr

The general best practice is: **your type declarations match your JavaScript distribution files**. So, 
...
- Your declaration files don't expose files that don't exist in you JavaScript bundle (this happens your your JavaScript is bundled but your declarations are not)

To answer whether you should bundle your declaration files is answering the question of whether you should bundle 
...
1. Our library is consumed by an environment that expects a bundled file (browser or cjs)
2. Consumers of our library do not use a bundler and require a small library footprint

## When to Keep Types Unbundled

Not bundling our types is a good default because:

1. Most consumer code usees a bundler before shipping to the browser run running on the server
2. We can use declaration maps to make navigating types easier

## A Special Case for the `exports` Property?

The [exports](https://nodejs.org/api/packages.html#package-entry-points) property of the `package.json` file 
allows defining [entry points] of a package. It's supported in Node.js 12+ and all 
...
When the `exports` property is specified import like `import { X } from 'lib/internal/foo'` will fail unless `lib/internal/foo` is a 
valid entrypoint listed in the `exports` property.

In many ways, defining a root entry point with the `exports` property `.` makes your library behave like it is bundled - even if it is not.
This is because the `exports` property hides internal APIs the same way bundling does.

When using the `exports` property, we have a stronger reason to use bundled types, since we're emulating the behavior 
...
adds less complexity to your build setup.
