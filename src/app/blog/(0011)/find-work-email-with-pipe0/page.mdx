...
using the pipe [PeopleGetWorkEmailWaterfall@1](https://www.pipe0.com/resources/pipe-catalog/people-get-work-email-waterfall/version-1).
...
const result = await fetch("https://api.pipe0.com/v1/run", {
  ...
  body: JSON.stringify({
    pipes: [{ name: "PeopleGetWorkEmailWaterfall@1", providers: ["findymail", "dropcontact"] }],
    input: [{id, firstName: "<FIRST_NAME>", lastName: "<LAST_NAME>", companyWebsiteURL: "<COMPANY_URL>"}]
  })
});
...
-d '{
    "pipes": [{ "name": "PeopleGetWorkEmailWaterfall@1",  providers: ["findymail", "dropcontact"] }],
    "input": [{"id": 1, "firstName": "<FIRST_NAME>", "lastName": "<LAST_NAME>", "companyWebsiteURL": "<COMPANY_URL>"}]
}'
```
    </div>
</CodeTabs>
