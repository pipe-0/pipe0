...
- 🧪 Processing individual rows and cells of tables
- ℁ Add columns with flexibility
- 😢 Processing with partially incomplete inputs
...
When you run a pipeline, the processing happens asynchronously. The pipeline response includes an `id` property which you can 
...
> response as a validation result. However, this result will neither have an `id` nor `status` property.
...
return the same response format: A <AppLink linkType="pipelineResponse">pipeline response</AppLink>.
...
If your pipeline was run the endpoint always returns an HTTP 200 OK status.
